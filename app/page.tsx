...
const formatClockTime = (value: string) => new Intl.DateTimeFormat("en-GB", {
  hour: "2-digit",
  minute: "2-digit",
  hour12: true,
  timeZone: "Asia/Colombo",
...
export default function Home() {
  ...
  const [selectedPresentDate, setSelectedPresentDate] = useState<string | null>(null);
  ...
  const todayRecords = attendanceRecords
    .filter((record) => colomboDateTimeParts(record.logTime).date === todayValue)
    .sort((left, right) => new Date(left.logTime).valueOf() - new Date(right.logTime).valueOf());
  const todayFirstRecord = todayRecords[0] || null;
  const todayLastRecord = todayRecords.length >= 2 ? todayRecords[todayRecords.length - 1] : null;
  const currentColomboMinutes = colomboDateTimeParts(new Date().toISOString()).minutes;
  const todayCalculationEndMinutes = todayLastRecord
    ? colomboDateTimeParts(todayLastRecord.logTime).minutes
    : currentColomboMinutes;
  const todayWorkMinutes = todayFirstRecord
    ? Math.max(0, todayCalculationEndMinutes - colomboDateTimeParts(todayFirstRecord.logTime).minutes)
    : 0;
  const todayWorkHours = Math.floor(todayWorkMinutes / 60);
  const todayRemainingMinutes = todayWorkMinutes % 60;
  const dailyTargetMinutes = 9 * 60;
  const todayWorkProgress = Math.min(100, Math.round((todayWorkMinutes / dailyTargetMinutes) * 100));
  const selectedDayRecords = selectedPresentDate
    ? attendanceRecords
      .filter((record) => colomboDateTimeParts(record.logTime).date === selectedPresentDate)
      .sort((left, right) => new Date(left.logTime).valueOf() - new Date(right.logTime).valueOf())
    : [];
  const selectedDayFirst = selectedDayRecords[0] || null;
  const selectedDayLast = selectedDayRecords.length >= 2 ? selectedDayRecords[selectedDayRecords.length - 1] : null;
  const selectedDayWorkMinutes = selectedDayFirst && selectedDayLast
    ? Math.max(0, colomboDateTimeParts(selectedDayLast.logTime).minutes - colomboDateTimeParts(selectedDayFirst.logTime).minutes)
    : 0;
  const selectedDayLabel = selectedPresentDate
    ? new Intl.DateTimeFormat("en-GB", { dateStyle: "full", timeZone: "Asia/Colombo" }).format(new Date(`${selectedPresentDate}T00:00:00+05:30`))
    : "";
  const [selectedCalendarYear, selectedCalendarMonthNumber] = selectedMonth.split("-").map(Number);
  const selectedCalendarMonthIndex = selectedCalendarMonthNumber - 1;
  const selectedCalendarLabel = new Intl.DateTimeFormat("en-GB", { month: "long", year: "numeric" })
    .format(new Date(selectedCalendarYear, selectedCalendarMonthIndex, 1));
  const currentMonthDays = new Date(selectedCalendarYear, selectedCalendarMonthIndex + 1, 0).getDate();
  const leadingCalendarCells = (new Date(selectedCalendarYear, selectedCalendarMonthIndex, 1).getDay() + 6) % 7;
    ...
    <main className="dashboard-shell employee-dashboard">
      ...
      <section className="dashboard-main"><header className="topbar"><div className="dashboard-logo-mobile"><img src="/cpstl-logo.png" alt="CPSTL" /></div><div className="status-pill"><i /> Live records · {lastUpdatedAt ? `Updated ${lastUpdatedAt.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}` : "Connected"}</div><div className="top-actions"><div className="avatar">{initials(employeeName)}</div><div className="user-name"><strong>{employeeName}</strong><span>{employee.epf ? `EPF ${employee.epf}` : `PIN ${employee.pin}`}</span></div><button className="mobile-signout" type="button" onClick={signOut} aria-label="Sign out">Sign out</button></div></header>
        <div className="dashboard-content"><div className="welcome-row"><div><p className="eyebrow">EMPLOYEE ATTENDANCE</p><h1>Welcome, {employeeName}.</h1><p>Your employee profile and fingerprint attendance records.</p></div></div>
          <form className="admin-filters panel" style={{ gridTemplateColumns: "220px 220px 110px" }} onSubmit={(event) => { event.preventDefault(); loadMyAttendance().catch((error) => setLoginError(error instanceof Error ? error.message : "Unable to load attendance records.")); }}><label>Starting month<select style={{ height: 40, padding: "0 11px", border: "1px solid #dfe2e7", borderRadius: 7, background: "white" }} value={selectedMonth} onChange={(event) => { setSelectedMonth(event.target.value); setSelectedPresentDate(null); }}>{monthOptions.map((month) => <option key={month.value} value={month.value}>{month.label}</option>)}</select></label><label>Period ends<input type="date" value={endDate} min={selectedMonthStart} max={todayValue} onChange={(event) => setEndDate(event.target.value)} /></label><button type="submit">Show</button></form>
          ...
          <div className="employee-grid"><section className="panel today-card"><div className="panel-title"><div><h2>Today&apos;s attendance</h2><p>Your live fingerprint status</p></div><span className={`attendance-tag ${todayRecords.length === 0 ? "half-day" : ""}`}>{todayRecords.length > 0 ? "Present" : "No record"}</span></div><div className="today-timeline"><div className={`timeline-event ${todayFirstRecord ? "done" : ""}`}><span>◎</span><div><small>CHECK IN</small><strong>{todayFirstRecord ? formatClockTime(todayFirstRecord.logTime) : "No record"}</strong><p>{todayFirstRecord ? `${todayFirstRecord.deviceLabel || todayFirstRecord.deviceSerial || "Fingerprint device"} · ${todayFirstRecord.deviceLocation || "Location unavailable"}` : "Waiting for first fingerprint"}</p></div></div><i /><div className={`timeline-event ${todayLastRecord ? "done" : ""}`}><span>◷</span><div><small>CHECK OUT</small><strong>{todayLastRecord ? formatClockTime(todayLastRecord.logTime) : "Pending"}</strong><p>{todayLastRecord ? `${todayLastRecord.deviceLabel || todayLastRecord.deviceSerial || "Fingerprint device"} · ${todayLastRecord.deviceLocation || "Location unavailable"}` : "Working time is calculating live"}</p></div></div></div><div className="worked-progress"><span><b>Time worked today</b><strong>{todayFirstRecord ? `${todayWorkHours}h ${todayRemainingMinutes}m / 9h` : "No records today"}</strong></span><div><i style={{ width: `${todayWorkProgress}%` }} /></div></div></section>
            <section className="panel calendar-card"><div className="panel-title"><div><h2>{selectedCalendarLabel}</h2><p>Select a present day to view its details</p></div></div><div className="calendar-head">{["MON","TUE","WED","THU","FRI","SAT","SUN"].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{currentMonthCalendar.map((item, index) => <div key={index} className={`${item ? item.state : "empty"}${item?.dateKey === selectedPresentDate ? " selected-present-day" : ""}`} role={item?.state === "present" ? "button" : undefined} tabIndex={item?.state === "present" ? 0 : undefined} aria-label={item?.state === "present" ? `View attendance details for ${item.dateKey}` : undefined} onClick={() => { if (item?.state === "present") setSelectedPresentDate(item.dateKey); }} onKeyDown={(event) => { if (item?.state === "present" && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); setSelectedPresentDate(item.dateKey); } }}>{item?.day}</div>)}</div><div className="calendar-legend"><span><i className="present" /> Present</span><span><i className="no-record" /> No record</span></div></section></div>
      ...
      {selectedPresentDate && <aside className="present-day-side-card" aria-label="Selected present day details"><header><div><p className="eyebrow">PRESENT DAY</p><h2>{selectedDayLabel}</h2></div><button type="button" aria-label="Close present day details" onClick={() => setSelectedPresentDate(null)}>×</button></header><div className="present-day-side-body"><div><span>First fingerprint</span><strong>{selectedDayFirst ? formatClockTime(selectedDayFirst.logTime) : "No record"}</strong></div><div><span>Last fingerprint</span><strong>{selectedDayLast ? formatClockTime(selectedDayLast.logTime) : "Only one scan"}</strong></div><div><span>Working time</span><strong>{selectedDayLast ? `${Math.floor(selectedDayWorkMinutes / 60)}h ${selectedDayWorkMinutes % 60}m` : "Not available"}</strong></div><div><span>Fingerprint records</span><strong>{selectedDayRecords.length}</strong></div>{selectedDayFirst && <section><small>First device</small><p>{selectedDayFirst.deviceLabel || selectedDayFirst.deviceSerial || "Not recorded"}</p><small>Location</small><p>{selectedDayFirst.deviceLocation || "Not recorded"}</p></section>}</div></aside>}
