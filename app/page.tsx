...
export default function Home() {
  ...
  return (
    <main className="dashboard-shell employee-dashboard">
      <aside className="sidebar"><div className="sidebar-brand"><img src="/cpstl-logo.png" alt="CPSTL" /><div><strong>CPSTL</strong><span>Employee Portal</span></div></div><nav><p>MY WORKSPACE</p><button className="nav-active"><span>▦</span> My Attendance</button><p>SUPPORT</p><button><span>?</span> Request Correction</button><button><span>☏</span> Help & Support</button></nav><div className="sidebar-bottom"><button><span>⚙</span> Settings</button><button onClick={() => setSignedIn(false)}><span>↪</span> Sign out</button></div></aside>
      <section className="dashboard-main"><header className="topbar"><div className="dashboard-logo-mobile"><img src="/cpstl-logo.png" alt="CPSTL" /></div><div className="status-pill"><i /> Attendance records connected</div><div className="top-actions"><button aria-label="Notifications">♢<b>1</b></button><div className="avatar">{initials(employeeName)}</div><div className="user-name"><strong>{employeeName}</strong><span>{employee.epf ? `EPF ${employee.epf}` : `PIN ${employee.pin}`}</span></div></div></header>
        <div className="dashboard-content"><div className="welcome-row"><div><p className="eyebrow">EMPLOYEE ATTENDANCE</p><h1>Welcome, {employeeName}.</h1><p>Your employee profile and fingerprint attendance records.</p></div><button className="outline-button">↥ Download my report</button></div>
