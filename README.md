...
To reset every active employee account to first-login mode (the employee can use
their PIN or EPF as the temporary password), force a password change, clear lockouts, and revoke current
sessions, run:

```powershell
npm run passwords:reset-all-employees
```

This bulk command does not reset administrator accounts. Use it only during an
authorized organization-wide password reset.

After a reset, the employee signs in as follows:

```text
Username: Employee PIN or EPF
Temporary password: Employee PIN or EPF
```

The temporary password is accepted only while `MustChangePassword = 1`. The
employee must immediately choose a private password.
